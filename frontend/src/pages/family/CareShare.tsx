...
export function CareShare({ member, shares, size }: CareShareProps) {
  ...
  const bar = (
    <span
      ...
      className={`flex overflow-hidden rounded-full bg-slate-200 shadow-inner ${size === 'lg' ? 'h-7 w-full max-w-64' : 'h-4 w-24'}`}
    >
      {anyDone &&
        shares.map((share) => (
          <span
  ...
  const badge = (
    <p
      ...
      aria-hidden="true"
    >
      <ScaleIcon className={size === 'lg' ? 'size-11' : 'size-7'} />
      <span className="flex flex-col leading-none">
        <span
          className={`font-extrabold text-slate-800 tabular-nums ${size === 'lg' ? 'text-4xl' : 'text-xl'}`}
        >
          {anyDone ? t('care.percent', { percent: own.percent }) : '–'}
        </span>
        <span className={`font-bold text-slate-600 ${size === 'lg' ? 'text-base' : 'text-xs'}`}>
          {t('care.this_week')}
        </span>
      </span>
    </p>
  ...
  // In der Spalte kompakt in einer Zeile, in der Personenansicht groß mit Legende.
  if (size === 'md') {
    return (
